...
describe('main', function () {
	...
	test('should run the executable', async function () {
		const bin = {
			...
			path: jest.fn().mockReturnValue('/bin/sauceconnect')
		}
		await main(bin, ['bar', '--foo'])
		expect(bin.run).toBeCalledTimes(1)
		expect(childProcess.spawn).toBeCalledWith('/bin/sauceconnect', ['bar', '--foo'], expect.any(Object))
	...
	describe('.binWrapper', function () {
		...
		test('should respect the SAUCECONNECT_INSTALL_BINARY wrapper', async function () {
			const bw = await main.binWrapper('http://some-fake-url');
			expect(bw._src).toMatchSnapshot();
			expect(typeof bw._dest).toEqual('string');
		});

		test('should respect the SAUCECONNECT_INSTALL_BINARY_MIRROR wrapper', async function () {
